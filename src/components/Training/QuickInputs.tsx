...
const QuickInputs = ({
  ...
  const step = (v: number, d: number): number => Math.max(0, Number((v + d).toFixed(1)));
...
const Input = ({ label, value, onInc, onDec }: { label: string; value: number; onInc: () => void; onDec: () => void }): JSX.Element => {
  return (
